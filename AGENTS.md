...
- Treat the running session daemon as independently deployed from the helper on disk. A compatible daemon with live sessions may continue running. Replace an empty stale daemon. Require explicit consent before an incompatible upgrade ends sessions, unless parallel daemon generations preserve them.
