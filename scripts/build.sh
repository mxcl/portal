...
Build and codesign InfiniTerm.app using an installed Developer ID identity.

Options:
  --install          Replace /Applications/InfiniTerm.app with the built app.
...
resolve_main_app_provisioning_profile() {
  ...
  if [[ -n "$profile" ]]; then
    ...
    [[ -f "$profile" ]] || die "InfiniTerm provisioning profile not found: $profile"
    profile_matches_main_app "$profile" ||
      die "InfiniTerm provisioning profile does not authorize $APP_BUNDLE_ID and $APP_KEYCHAIN_ACCESS_GROUP: $profile"
    printf '%s\n' "$profile"
    return 0
  fi
...
generate_release_plan() {
  ...
  if previous_tag="$(latest_release_tag)"; then
    ...
    prompt="Plan the next InfiniTerm release.
...
<X.Y.Z>"
    ...
    prompt="Plan the initial InfiniTerm release.
...
echo "Building InfiniTerm app bundle"
rm -rf "$APP_DIR"
mkdir -p \
  "$MACOS_DIR" \
  "$RESOURCES_DIR" \
